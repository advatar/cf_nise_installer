...
(
    ...
    bundle install
    # Old spec format
     env PATH=$PATH bundle exec ./bin/nise-bosh -y ../cf-release ../.deploy.yml micro -n ${NISE_IP_ADDRESS}
    # New spec format, keeping the  monit files installed in the previous run
     env PATH=$PATH bundle exec ./bin/nise-bosh --keep-monit-files -y ../cf-release ../.deploy.yml micro_ng -n ${NISE_IP_ADDRESS}
)
